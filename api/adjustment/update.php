...
if( //$data->batch_id != NULL
	...
	if($adjust->clear($data[0]->batch_id)){
		//set response code 201
        http_response_code(201);
        //return success message to user
        echo json_encode(array("message" => "Adjustments cleared successfully"));
      }else{
        ...
        echo json_encode(array("message" => "Adjustments clear failed"));
	}
	
	foreach($data as $obj){
	  $adjust->batch_id = $obj->batch_id;
	  $adjust->adjust_type = $obj->adjust_type;
	  $adjust->adjust_text = $obj->adjust_text;
	  $adjust->chronology = $obj->chronology;

	  if($adjust->createWithID()){
